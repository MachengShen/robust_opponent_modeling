...
class SSNE:
			...
			if len(W1.shape) == 2: #Weights no bias
				...
				for i in range(num_cross_overs):
					...
					if receiver_choice < 0.5:
						...
						W1[ind_cr, :] = W2[ind_cr, :]  #ms: copy the weight parameter of some layer of one nn to another?
					else:
						ind_cr = random.randint(0, W1.shape[0]-1)  #
						W2[ind_cr, :] = W1[ind_cr, :]
	...
	def mutate_inplace(self, gene):
		"""Conduct mutation in place  #ms: mutation means perturb parameter
	...
	def epoch(self, gen, genealogy, pop, net_inds, fitness_evals, migration):
		...
		if num_elitists < 1: num_elitists = 1  #macheng: was 2
		...
		#here is the synchronization step
		for policy in migration:
			replacee = unselects.pop(0)   #macheng: this means unselects must at least size len(migration) ?
