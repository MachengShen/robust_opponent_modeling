...
class World(object):
    ...
    def step_discrete(self):
        ...
        #no need to apply envionment force, constraint agent into a fitnite grid world
        self.discrete_integrate_state(p_force)
    ...
    def discrete_integrate_state(self, p_force):
        for i, entity in enumerate(self.entities):
            if not entity.movable:
                continue
            if "box" in entity.name and not self.check_all_agent_in_contact(entity):
                continue
            entity.state.p_pos += p_force
            #constrain into grid world
            self.constrain(entity.state.p_pos)

    def constrain(self, p_pos):  #clip the xy location s.t. within the grid
        p_pos[0] = np.clip(p_pos[0], self.x_range[0], self.x_range[1])
        if p_pos.shape[0] == 2:
            p_pos[1] = np.clip(p_pos[1], self.y_range[0], self.y_range[1])
        else:
